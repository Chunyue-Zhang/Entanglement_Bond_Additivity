...
def create_RPS_using_rand(  # RPS指random product state，rand指[0,1)上均匀分布的随机实数
    rand_array,  # 由rand组成的.shape=(num_qubits,2)的numpy数组
):
    num_qubits = rand_array.shape[0]
    rps = 1.0
    for i in range(num_qubits):
        theta = np.arccos(1 - 2 * rand_array[i, 0])
        phi = 2 * np.pi * rand_array[i, 1]
        rps = np.kron(
            rps, np.array([np.cos(theta / 2), np.exp(1j * phi) * np.sin(theta / 2)])
        )
    return rps  # 1维numpy数组形式
...
##其他（包括数据类型不确定的变量）----------
BC = "PBC"
num_particles = None  # particle number
pauli = False
...
for i in range(num_pre_samples):
    print(i)
    # 随机构型
    p0r = rng.uniform(size=(L, 2))
    hs = rng.uniform(low=-W, high=W, size=(L,))
    hs_x = rng.uniform(low=-W, high=W, size=(L,))
    # 计算
    psi0_vector = create_RPS_using_rand(  # RPS指random product state，rand指[0,1]上均匀分布的随机实数
        p0r,  # .shape=(num_qubits,2)的numpy数组
    )
    h_list = [[hs[ii], ii] for ii in range(L)]
    h_x_list = [[hs_x[ii], ii] for ii in range(L)]
    ...
    EE = calculate_bipartitions_EE_dynamics(
        ...
        fulle=fulle,
        fullv=fullv,
        subsys_spin_idx_list=mut_info_related_subsys_spin_idx_list,  # subsystem spin index，以元组形式存在列表中
    )
    # 收集数据
    psi0_rand.append(p0r)
    disorder.append(hs)
    disorder_x.append(hs_x)
